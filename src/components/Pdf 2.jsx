...
//   // pdfjs v5 uses mjs worker; keep this line for client-only assignment
...
//   margin: 0 clamp(10px, ${() => viewSizeCalculator(16, true)}, 16px)
//     clamp(10px, ${() => viewHeightCalculator(16, true)}, 16px) 0;
...
//   /* 👇 top-left corner straight if first active */
//   border-top-left-radius: ${(p) =>
//     p.$isFirstActive ? "0" : `clamp(14px, ${viewSizeCalculator(20, true)}, 20px)`};

//   /* 👇 bottom-left corner straight if active (in wide mode) */
...
//   width: 100%;
//   height: clamp(150px, ${() => viewHeightCalculator(407, true)}, 450px);
...
//         list.push({ num: i, data: canvas.toDataURL() });
//       }
...
//         <ThumbItem
//           key={t.num}
//           $active={t.num === currentPage}
//           onClick={() => onPageClick(t.num)}
//         >
//           <img src={t.data} alt={`Page ${t.num}`} />
//           <span>{t.num}</span>
//         </ThumbItem>
//       ))}
//     </ThumbColumn>
...
//   border: 2px solid ${(p) => (p.$active ? "#d63384" : "#ccc")};
//   border-radius: 4px;
//   cursor: pointer;
//   padding: 4px;
//   background: white;
...
//   }
// `;
// const PDFPageCanvas = ({ pdfUrl, pageNumber, onPageInView }) => {
...
//   // Render all pages at their natural size (scale 1.0)
//   useEffect(() => {
...
//         const canvas = containerRef.current.querySelector(`#page-${i}`);
...
//   // Scroll to clicked thumbnail page
//   useEffect(() => {
//     if (!pageNumber || !pageRefs[pageNumber - 1]) return;
//     pageRefs[pageNumber - 1].scrollIntoView({ behavior: "smooth", block: "start" });
//   }, [pageNumber, pageRefs]);

//   // Track visible page (update active thumbnail)
//   useEffect(() => {
//     if (!containerRef.current || pageRefs.length === 0) return;
...
//         if (visible && onPageInView) {
//           const num = parseInt(visible.target.dataset.page, 10);
//           onPageInView(num);
//         }
//       },
//       {
...
//   }, [pageRefs, onPageInView]);

//   return (
//     <PageCanvasContainer ref={containerRef}>
//       {Array.from({ length: numPages }, (_, i) => (
//         <CanvasWrapper key={i + 1}>
//           <canvas id={`page-${i + 1}`} data-page={i + 1} />
//         </CanvasWrapper>
//       ))}
//     </PageCanvasContainer>
//   );
...
//         height:
//           sectionRect.height - paddingTop - paddingBottom - borderTop - borderBottom,
//       });
//     };
